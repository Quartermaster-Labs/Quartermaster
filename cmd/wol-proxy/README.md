...
wol-proxy automatically wakes up a suspended quartermaster server using Wake-on-LAN when requests are received.

When a request arrives and quartermaster is unavailable, wol-proxy sends a WOL packet and holds the request until the server becomes available. If the server doesn't respond within the timeout period (default: 60 seconds), the request is dropped.
