...
llama-quartermaster is designed to be very simple: one binary, one configuration file.
...
This is enough to launch `llama-server` to serve `model1`. Of course, llama-quartermaster is about making it possible to serve many models:
...
llama-quartermaster is also about customizability. You can use any CLI flag available:
...
llama-quartermaster supports any OpenAI API compatible server. If you can run it on the CLI llama-quartermaster will be able to manage it. Even if it's run in Docker or Podman containers.
...
llama-quartermaster supports many more features to customize how you want to manage your environment.
...
> [!NOTE]
> Always check [`config.example.yaml`](../config.example.yaml) for the most up to date reference for all example configurations.

```yaml
# add this modeline for validation in vscode
# yaml-language-server: $schema=./config-schema.json
#
# llama-quartermaster YAML configuration example
# -------------------------------------
#
...
# Usage notes:
# - Below are all the available configuration options for llama-quartermaster.
...
# - valid values:
#   - "proxy": logs generated by llama-quartermaster when swapping models,
...
# - optional, default: []
# - when empty (the default) authorization will not be checked as llama-quartermaster is default-allow
...
models:
  ...
  "gpt-oss-120b":
    ...
    # proxy: the URL where llama-quartermaster routes API requests
  ...
  "docker-llama":
    ...
    #
    # When empty, llama-quartermaster has this default behaviour:
...
# - optional, default empty dictionary
# - peers can be another llama-quartermaster
# - peers can be any server that provides the /v1/ generative api endpoints supported by llama-quartermaster
peers:
  # keys is the peer'd ID
  llama-quartermaster-peer:
    # proxy: a valid base URL to proxy requests to
    # - required
    # - requested path to llama-quartermaster will be appended to the end of the proxy value
