...
quartermaster is designed to be very simple: one binary, one configuration file.
...
This is enough to launch `llama-server` to serve `model1`. Of course, quartermaster is about making it possible to serve many models:
...
quartermaster is also about customizability. You can use any CLI flag available:
...
quartermaster supports any OpenAI API compatible server. If you can run it on the CLI quartermaster will be able to manage it. Even if it's run in Docker or Podman containers.
...
quartermaster supports many more features to customize how you want to manage your environment.
...
#
# quartermaster YAML configuration example
# -------------------------------------
#
...
# Usage notes:
# - Below are all the available configuration options for quartermaster.
...
# - valid values:
#   - "proxy": logs generated by quartermaster when swapping models,
...
# - optional, default: []
# - when empty (the default) authorization will not be checked as quartermaster is default-allow
...
models:
  ...
  "gpt-oss-120b":
    ...
    # proxy: the URL where quartermaster routes API requests
  ...
  "docker-llama":
    ...
    #
    # When empty, quartermaster has this default behaviour:
...
# - optional, default empty dictionary
# - peers can be another quartermaster
# - peers can be any server that provides the /v1/ generative api endpoints supported by quartermaster
peers:
  # keys is the peer'd ID
  quartermaster-peer:
    # proxy: a valid base URL to proxy requests to
    # - required
    # - requested path to quartermaster will be appended to the end of the proxy value
