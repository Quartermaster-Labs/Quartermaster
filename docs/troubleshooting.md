...
- **"Open models folder" does nothing, or says xdg-open is missing** -> that button asks the SERVER to open a file manager, which only works when you are sitting at the machine Quartermaster runs on. Headless (a container, or the dashboard open from another computer) there is no desktop to open it on, so the button browses the models tree inside the app instead: folders, sizes and a copy-path button for each row. It is read-only.
