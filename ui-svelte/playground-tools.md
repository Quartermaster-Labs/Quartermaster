...
| `youtube.ts` | `YOUTUBE_TOOL` (advertised as **`media_transcript`** — yt-dlp is not a YouTube client, so it takes any video/audio page: Vimeo, TED, Twitch VODs, podcast episodes…) / `YOUTUBE_SEARCH_TOOL` / `YOUTUBE_COMMENTS_TOOL` (those two stay YouTube-only: `ytsearch:` and comment extraction have no equivalent elsewhere), fetched server-side. Only `media_transcript` is always advertised (it fires on a link the user pasted); search and comments ride the Web Search toggle, since they are lookups the user did not bring up and cost prefix in every chat. `DEFAULT_YOUTUBE_PROMPT` names no tool but `media_transcript` for that reason, and the per-tool how-to lives in the schemas: a prompt line and a tool description gated on the same flag only pay for the same guidance twice. Search and channel-listing are ONE tool on purpose: they return the same shape, and a weak local model choosing between two near-identical tools mostly chooses wrong. Plus link unfurling — `extractYouTubeIds` finds video links in a message, `fetchYouTubeMeta` resolves them through `/api/youtube/meta` with a module-level per-id promise cache, rendered by `playground/YouTubeEmbed.svelte`. |
