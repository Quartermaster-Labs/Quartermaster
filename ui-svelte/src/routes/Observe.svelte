<script lang="ts">
  ...
  import { Activity as ActivityIcon, ScrollText, Gauge } from "lucide-svelte";
  ...
  // cutoff). Logs are an unbounded stream, so hide the selector there.
  const showWindow = $derived($observeTab !== "logs");
</script>
