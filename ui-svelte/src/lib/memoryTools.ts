...
export const MEMORY_SAVE_TOOL: ToolDef = {
  ...
  function: {
    name: "memory_save",
    description:
      "Remember one durable fact about this user across conversations. Save when they ask you to remember something, or state a lasting preference, constraint or detail about themselves or their setup that would change future answers - as soon as you see it, without checking your memory block first (a restatement in similar words is merged automatically). If the result says an existing memory is close, settle it in this turn: same fact means saving one combined text onto the OLDER memory's id, then memory_delete the new one; different facts, keep both. Never save what only matters to this conversation or can be looked up at any time. To fix a wrong or outdated memory, pass its `id` to replace it. One fact per call, third person, self-contained.",
    parameters: {
      type: "object",
      properties: {
        text: {
