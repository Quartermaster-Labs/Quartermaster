<script lang="ts">
  ...
  function handleTitleChange(newTitle: string): void {
    const sanitized = newTitle.replace(/\n/g, "").trim().substring(0, 64) || "Quartermaster";
...
>
  {#if $screenWidth !== "xs" && $screenWidth !== "sm"}
    <h1
      ...
      class="!text-xl p-0 outline-none hover:bg-secondary rounded px-1"
      onblur={handleBlur}
  ...
  <menu class="flex items-center gap-4 overflow-x-auto pretty-scroll">
    ...
    <button onclick={toggleTheme} use:tip={`Toggle theme (current: ${$themeMode})`}>
      {#if $themeMode === "system"}
