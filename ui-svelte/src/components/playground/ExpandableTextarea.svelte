...
<div class="flex-1 relative group flex items-stretch min-h-0">
  <textarea
    class="w-full px-3 py-2 pr-10 rounded border border-card-border bg-surface focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary resize-none"
    {placeholder}
    {rows}
    bind:value
    {onkeydown}
    ...
    <div class="w-full max-w-4xl h-[80vh] flex flex-col bg-surface rounded-lg shadow-xl border border-card-border">
      ...
      <div class="flex-1 p-4">
        <textarea
          ...
          class="w-full h-full px-4 py-3 rounded border border-card-border bg-card focus:outline-none focus:ring-2 focus:ring-primary resize-none"
      ...
      <!-- Footer -->
      <div class="flex justify-end gap-2 p-4 border-t border-card-border">
        <button
          class="btn"
          onclick={closeExpanded}
          type="button"
