...
| audio.cpp catalog | `internal/audiocpp/specs.go` (package doc) | Reads the `model_specs/` catalog that ships INSIDE an installed audio.cpp, so the browser can offer its ~70 families as downloads |
