...
| `dialguard.go` | The caller-origin rule for web search. `PublicOnly(ctx)` marks a request as untrusted, and the shared `webSearchClient` transport refuses any dial under such a context that lands on a non-public address. |
...
| `youtube.go` | Transcript fetching for **any** site yt-dlp handles (tool `media_transcript`), not just YouTube: `ParseMediaTarget`/`ParseVideoID`, `DlpPath` (managed yt-dlp), `GetTranscript` (exec, auto-caption VTT → ~30s paragraphs, token-budgeted truncation), `FormatTranscript`. |
...
- `ParseMediaTarget(ctx, s)` — the gate in front of yt-dlp. A bare YouTube id or any YouTube URL canonicalises to `https://www.youtube.com/watch?v=<id>` (so the cache keys once per video); anything else must be an `http(s)` URL with no userinfo, a dotted hostname, port empty/80/443, and an address that passes `shared.IsPublicIP` — an IP literal directly, a hostname through a 5s DNS lookup where **every** returned address must be public. Returns `MediaTarget{URL, ID, Site}`; `ID` is empty off YouTube, which is what makes the header drop the "YouTube" wording and the `&t=` deep-link hint. Front-door check only: yt-dlp resolves and follows redirects itself, so unlike `fetch_page` (whose dialer hook covers rebinding) this cannot police what the downloader does after the first hop.
...
- `GetTranscript(ctx, target, lang)` — `target` is anything `ParseMediaTarget` accepts (URL on any site, or a bare YouTube id); the cache keys on the canonical URL. `yt-dlp --skip-download --write-auto-subs`, then the subtitle file is parsed: auto-caption VTT is a rolling window (every cue repeats the previous cue's last line, words carry inline `<time><c>` markup), so cues are folded into ~30s paragraphs with one timestamp each — that is the whole point of the tool being affordable in context. `lang` is validated against a strict code regex **before** it reaches argv. The result is token-budgeted (`ytMaxTokens`, 12k by default); truncation keeps whole paragraphs and reports the timestamp it cut at, so the model can say what it is missing.
- `FormatTranscript(tr, citation, maxTokens)` — off YouTube (`tr.ID == ""`) the header reads "Transcript from <site>" and offers no `&t=` link, because only YouTube has a documented seek parameter and a fabricated one 404s. `citation` 0 means "no number" (the API path); `maxTokens` 0 means the default ceiling, and an over-large ask is **clamped to the ceiling, not honoured**.
...
- **Web search takes its endpoint from the caller, which is both the feature and an SSRF.** A SearXNG instance is personal and usually on loopback or the LAN, so the target cannot simply be restricted. The split is by ORIGIN instead: `internal/server/websearch.go`'s `guardSearchCtx` reads `r.RemoteAddr` (never `X-Forwarded-For`, which the client writes) and marks anything non-loopback `PublicOnly`. Every handler that reaches the chain must call it: `/api/websearch` (both shapes), `/v1/tools/search`, and the turn runner, which has to take the verdict while the request still exists because the turn outlives it. The check itself sits in `net.Dialer.Control`, after DNS and once per attempt, so a rebinding host and a redirect to a private address are both caught; the proxy is dropped for guarded callers, since a proxy dial would only ever show the proxy's own address.
