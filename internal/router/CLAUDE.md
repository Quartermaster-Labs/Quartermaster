...
**The live VRAM ceiling (fork).** `groupSwapper.budgetEviction` no longer admits against the raw static budget: `effectiveBudgetGB` (`group.go`) tightens `vramBudgetGB` to a live VRAM ceiling supplied by the server via `LocalRouter.SetLiveVramBudget` (`router.LiveVramFn`, held in a `liveVramHolder` — a lock-free read, since this runs on every request and every queue drain). The ceiling is `vramBudgetGB − max(0, foreign − foreignFloor) − oomGuardReserveGB` — foreign *growth over an idle baseline*, not raw free VRAM, so a card legitimately full of our own well-planned models reads as healthy (`excess == 0` returns the static budget verbatim) while a game claiming 8 GB of a 24 GB card shrinks the admission budget to its true headroom instead of letting the router evict residents to make room that isn't there (where the spawn guard in `autogen.LiveOffloadArgs` would then refuse the load anyway — both models gone). The probe reports `ok=false` when no trustworthy reading exists (no telemetry, or per-process attribution that can't see our own children); a missing or untrustworthy reading is *not* a reason to fall back to the static group policy — it just leaves the static budget in force, the behaviour that shipped before the probe existed. `OnSwapStart` logs the tightening. See `internal/autogen/liveoffload.md` for the server-side guard (sampling, watchdog, refusal memo).
