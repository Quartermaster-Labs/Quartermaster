...
| `flagtable.go`, `flagtable_sd.go`, `customargs.go`, `pins.go`, `validate.go`, `backendhelp.go` | Custom launch arguments: a per-backend flag table (`LlamaFlags`, `SdFlags`) drives knob suppression (`ComposeCmd`), the editor's ownership locks, and validation; `PinsFromArgs` folds a pinned `-c`/`-ctk`/`-ub`/`-ngl`/... into the sizer so emitted flags and estimates describe the launch that runs; `BackendFlags` probes a backend's `--help` for the union. → [`../ui-svelte/launch-args.md`](../ui-svelte/launch-args.md) |
| `overrides.go` | Control-file types (`GenerateFile`, `Settings`, `Override`, `VariantSpec`, `GroupSpec`), defaults, loading/merging, `globLike` (PowerShell `-like`). `Settings.RootList()`/`CategoryRoots`/`CategoryOrder`/`IsCategory` = multi-root scan folders, one optional extra root per UI tab (**`CategoryOrder` must mirror `MODEL_CATEGORIES` in `ui-svelte/src/lib/modelUtils.ts`** - the picker renders on every tab, so a missing category is a root that is set, shown, and never scanned; a test enforces it and `/api/settings/root/pick` 400s on an unknown one). `Settings.LoraDirs` = the per-category LoRA folder; `"image"`/`"video"` are sd-server's `--lora-model-dir`, `"llm"` is only a base path (see `llmlora.go`). `Override` carries granular DRY, granular spec, the image fields (including the per-model `PromptEnhancerPrompt`/`PromptEnhancerEditPrompt` twins), and `Loras` (`LoraRef`, whose `Scale` is a POINTER because scale 0 means loaded-but-inert, not unset). |
...
5. **Emit** — `emitProfile` writes each model's YAML `cmd` block. Per-model engine knobs on
   ...
   pointers, and 0 is a value). `Override.CustomArgs` is composed in
   (`ComposeCmd`), which suppresses every generated flag whose knob the text owns; `ExtraArgs` is
   the legacy bucket and is still appended verbatim, but only by the emitters that have no flag
   table yet (audio, SAM, vllm). llama-server and sd-server both compose.
...
- `Settings.autoEnhancers` (unexported, `promptenhancer.go`): the pairing table `Generate` builds
  ...
  `resolveEnhancerIDs` reads it at emit time: a configured id wins (including `PEDisabled`), an
  empty one takes the detected id for its direction. Detection is NAME-only because a PE gguf
  reports `general.architecture=qwen3vl`, identical to every other VL chat model, so the arch
  gate the asr/tts/embedding classifiers rely on says nothing here. The rule is deliberately
  narrow: a `pe` token must be immediately followed by a `t2i`/`i2i` token, or the name spells
  `promptenhanc`, so "Pegasus-7B" is not reclassified out of the encoder pool. The family key is
...
- `Override.PromptEnhancer` / `Override.PromptEnhancerEdit`: which entry an image model opts
  ...
  text one composes a scene from nothing. Empty means "let detection fill this direction", and a
  missing edit half still falls back to the text one on the client. `PEDisabled` (`"none"`) is
  how a user refuses: auto-pairing gave the empty string a second meaning, so clearing a field
  with a detected candidate would be undone by the next regen. `resolveEnhancerIDs` preserves the
  sentinel and `writeEnhancerKey` drops it, which is what makes the refusal stick.
- `Override.PromptEnhancerPrompt` / `PromptEnhancerEditPrompt` (the `ExtraImageModel` struct has
  the same twins): the image model's OWN system prompt per direction, emitted by
  `writeEnhancerPrompt` with the same `yaml.Marshal` + `indentYAML` treatment as the table,
  because the prompt is written for the target model. One rewriter serves checkpoints that want
  very different output (SDXL tag soup, a Flux paragraph, a Qwen instruction), so the prompt
  belongs to the image model and outranks the settings row's at serve time. Skipped when the
  model has no enhancer in that direction. `internal/config.ModelConfig` carries both too.
...
`writeEnhancerBlock` is the entry the emitters call (`emitImageModel`, `emitExtraImageModels`):
it resolves the pair, then writes both ids plus both per-model prompts. `writeEnhancerKey` no
longer requires a `settings.promptEnhancers` row. An id with no row, whether typed by hand or
detected from a filename, is emitted as typed (a row only fixes up the id's canonical casing),
because dropping it hid the button with nothing to explain why. So a dangling id is not an error
any more: it runs with the model's own prompt, else with no system prompt at all. The rewrite
itself never happens server-side; see `internal/server/CLAUDE.md` and the playground's
`lib/promptEnhance.ts`.

The encoder pool is the other half of the same classifier. `IsPromptEnhancerFile` drops a PE
