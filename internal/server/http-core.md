...
| `backendmetrics.go` | Fork: `backendMetricsMonitor` — polls each running llama-server's `/metrics`+`/slots` on a 2s ticker for KV-fill / slot-saturation / throughput gauges, **skipped while busy** (both share llama-server's inference task queue; `RequestsProcessing` comes from quartermaster's own in-flight counter instead). `/props` (n_ctx, total_slots) is read on every idle tick: it describes the current load, and a model keeps its port across a reload, so caching it per base URL showed the previous load's window (`5k` while `/props` said `118784`). Caches per-model, emits `BackendMetricsEvent` over SSE; `GET /api/backend-metrics` snapshot. |
...
| `flagcheck.go` | Fork: boot check for the launch-arguments redesign (see `ui-svelte/launch-args.md`). `launchFlagWarnings` walks every llama-server command in the running config, probes the binary's `--help` (`autogen.BackendFlags`, memoised) and reports flags that make the spawn fail: one the emitter writes that this build no longer lists, or one neither the table nor the binary knows. Non-llama commands (sd-server, tts-server, ...) are skipped, a failed probe stays silent, and the lines are logged as WARN so the dashboard log shows them. |
...
| `appwindow.go` | Fork: `GET /api/app/show` — raises the native desktop window (`-app`); `showAppWindow` hook + `SetShowAppHook`. Loopback-checked against `r.RemoteAddr` ONLY (never the forwarded-for headers) and 404 when there is no window, so a LAN/tailnet host can neither pop a window nor learn one exists. The `{"app":"quartermaster"}` marker in the body is what a second launch verifies before deciding the port is ours and exiting — see `cmd/quartermaster/singleinstance.go`. Also `POST /api/app/quit` — graceful shutdown for the installer, gated on loopback + POST + an `X-Quartermaster-Quit` header (the header forces a CORS preflight, so no page can reach it cross-origin); replies with the pid, runs `shutdownHook` in a goroutine so the reply is written first. Client: `cmd/quartermaster/quit.go` (`-quit`). |
