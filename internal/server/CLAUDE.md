...
- **Auth scope.** API keys are checked by `CreateAuthMiddleware`; with no configured keys it is a pass-through. Every route (including `/api/*` and `/ui/`) is wrapped by it. Keys accepted via `Authorization: Bearer`, `Authorization: Basic` (password field), or `x-api-key`.
