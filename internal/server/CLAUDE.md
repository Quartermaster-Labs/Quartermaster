...
| `slotcache.go` | Fork: slot KV-cache persistence — saves a llama-server slot's KV to disk so an expensive conversation survives eviction, and seeds cold/warm loads from a per-agent system+tools **preamble cache**. This file holds the state machine (`middleware`, `onSwitch`, `restoreOnLoad`, `saveOnEvict`, `ensurePreambleSeed`, `synthPrefill`) and the `/slots` HTTP calls. See "Slot KV cache" below. |
...
| `youtube.go` | Fork: the `youtube_transcript` tool's fetch path (no HTTP route — it is only reachable through the `turns.go` tool loop). `parseYouTubeID` accepts watch/`youtu.be`/shorts/embed/bare-id; **exec-per-request** `yt-dlp --skip-download --write-subs --write-auto-subs --sub-format vtt` into a temp dir (no `--convert-subs`: that would need ffmpeg), binary resolved from PATH or beside the exe. `vttToParagraphs` strips cue timings + karaoke `<c>` markup and the auto-caption rolling repeat, merging into ~30s `[m:ss]` paragraphs (raw VTT is 2–3× the tokens of the words). `formatYouTubeTranscript` adds the citable header and truncates at `ytMaxTokens` with an explicit INCOMPLETE marker. 30-min cache, `maxYouTube` per turn, `ytTimeout`; video id + lang are regex-validated before reaching argv. |
| `youtube_meta.go` | Fork: `GET /api/youtube/meta?id=` — Discord-style link unfurl for the chat. **Not yt-dlp**: one unauthenticated YouTube **oEmbed** GET (title + channel), because unfurling fires on every pasted link while caption pulls are rate-limited (429s). 24h/256-entry cache, id regex-validated (`ytVideoID`), full URLs also accepted via `parseYouTubeID`. No duration (oEmbed has none). Thumbnails are **hotlinked** from `i.ytimg.com` by the browser, not proxied — a deliberate outbound-to-Google tradeoff; proxy via `extractMedia` if an offline/locked-down deployment needs it. |
...
- Context / observe extras: `GET /api/canon` (prompt-canon snapshot, `promptcanon.go`), `GET /api/backend-metrics` (`backendmetrics.go`), `GET /api/websearch` (SearXNG proxy, `websearch.go`), `GET /api/youtube/meta` (link-unfurl card metadata, `youtube_meta.go`).
