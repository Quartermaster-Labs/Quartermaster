...
- `ParseCmd(cmd)` / `CmdInfo` — `cmdinfo.go`. Memoized, read-only parse of a rendered launch command: `Argv`, `ModelPath` (`-m`/`--model`/`--model-path`/`--diffusion-model`, slash-normalized — the fork's model "family" key; a backend whose model flag is missing from `modelPathFlags` gets no family key AND cannot be saved from the config editor, which 400s with "model has no gguf path to override"), plus token-exact accessors `Has`/`Value`/`Values`/`Int`/`HasValue` (all handle `--flag=value`). Use this instead of re-splitting or substring-sniffing a command.
