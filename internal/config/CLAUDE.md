...
- `ParseCmd(cmd)` / `CmdInfo` — `cmdinfo.go`. Memoized, read-only parse of a rendered launch command: `Argv`, `ModelPath` (`-m`/`--model`/`--model-path`/`--diffusion-model`, else vllm's positional `<exe> serve <model>`, slash-normalized — the fork's model "family" key; a backend whose model flag is missing from `modelPathFlags` gets no family key AND cannot be saved from the config editor, which 400s with "model has no gguf path to override"), plus token-exact accessors `Has`/`Value`/`Values`/`Int`/`HasValue` (all handle `--flag=value`). Use this instead of re-splitting or substring-sniffing a command.
...
- **`vramBudgetGB` (top level) + `ModelConfig.EstVramGB` are the multi-load admission pair (fork).** Both are pure data here — the config layer only carries them; the policy lives in `internal/router/group.go` and the numbers are emitted by `internal/autogen`. `vramBudgetGB: 0` (the default, and every hand-written config) keeps the legacy static swap/exclusive behaviour untouched, and a model with `estVramGB: 0` is **unknown, not free** — the router refuses to admit it against a budget and falls back to the static policy. Don't default either field to a guess.
