...
| File | Role |
|---|---|
| `chain.go` | Defines the `Middleware` type and the `Chain` builder (`New`, `Append`, `Then`, `ThenFunc`). |
...
- `New(mws ...Middleware) Chain` — `chain.go:33`. Builds a chain; copies the slice so callers can't mutate it afterward.
- `(Chain) Append(mws ...Middleware) Chain` — `chain.go:42`. Returns a new chain with extra middleware appended; the receiver is unchanged, so a base chain can be safely reused across routes.
- `(Chain) Then(final http.Handler) http.Handler` — `chain.go:52`. Wraps `final` and returns the composed handler.
- `(Chain) ThenFunc(f http.HandlerFunc) http.Handler` — `chain.go:61`. Shorthand for `Then(http.HandlerFunc(f))`.
