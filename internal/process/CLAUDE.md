...
| `libpath_unix.go` / `libpath_windows.go` | (`!windows` / `windows`) `exeLibEnv` — puts the backend exe's own dir on `LD_LIBRARY_PATH` at spawn so self-contained bundles' transitive .so deps resolve (no-op on Windows, whose loader already searches the exe's dir). |
| `treecleanup_other.go` | (`//go:build !windows`) `SetupTreeCleanup` no-op — orphan cleanup handled by process groups. |
| `treecleanup_windows.go` | (`//go:build windows`) `SetupTreeCleanup` assigns the process to a Job Object with `KILL_ON_JOB_CLOSE` (plus `BREAKAWAY_OK`) so children die with quartermaster. |
...
- **Build tags.** `runtime_unix.go`/`treecleanup_other.go` are `!windows`; `runtime_windows.go`/`treecleanup_windows.go` are `windows`. `setProcAttributes`, `terminateProcessTree`, `killProcessTree`, `SetupTreeCleanup`, and `exeLibEnv` each have one definition per platform — keep the signatures identical across both.
- **Backend .so resolution (unix).** Self-contained bundles (llama.cpp Vulkan/ROCm tarballs) use `RUNPATH=$ORIGIN`, which covers only DIRECT deps; transitive ones (e.g. `libllama-server-impl.so` → `libllama-common.so.0`) fail with "error while loading shared libraries" unless a system ldconfig entry exists. `doStart` therefore prepends the resolved exe's directory to `LD_LIBRARY_PATH` (`DYLD_LIBRARY_PATH` on darwin) via `exeLibEnv`, skipping the loader's default dirs (`/usr/bin`, `/usr/lib`, ...) so system-installed backends keep their current resolution. This is the in-app twin of the Windows `peimports` early-exit hint — both exist because a missing runtime dependency is the #1 backend-install failure and the default error message blames nothing.
...
- **The job is `BREAKAWAY_OK`, and the self-updater is why.** Job membership is inherited, and `KILL_ON_JOB_CLOSE` does not care that a child is a whole new quartermaster: the updater's relaunch (`internal/update`, `Spawn`) joined this job and was killed the instant the outgoing process exited, so an update looked exactly like the app quitting. `BREAKAWAY_OK` changes nothing about a normal spawn (a backend still lands in the job and is still reaped): it only makes `CREATE_BREAKAWAY_FROM_JOB` legal for the caller that asks for it.
