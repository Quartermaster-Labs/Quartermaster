...
For the dynamic-spawn-flags goal, the relevant injection point is `doStart` → `SanitizedCommand()` (`process_command.go:362`), where `${CTX}/${NGL}/${NCPUMOE}/${CTK}/${CTV}` should be resolved at spawn time. Processes are intended to be keyed by `(modelID, ctx)` so different context sizes map to distinct `ProcessCommand` instances; the `id` field is the natural carrier of that composite key.
